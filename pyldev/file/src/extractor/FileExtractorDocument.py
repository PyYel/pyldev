...
class FileExtractorDocument(FileExtractor):
    ...
    def __init__(
        ...
        self.logger = _config_logger(
            logs_name="FileExtractorDocument",
            logs_output=["console"]
        )
    ...
    def _extract_pdf(self, file_path: str, text_threshold: int = 20) -> List[dict[str, Any]]:
        """
            ...
            try:
                ...
                for table_num, table in enumerate(table_finder.tables):
                    ...
                    try:
                        ...
                        print(f"  Warning: Could not extract table {table_num} on page {page_num}: {e}")
            ...
            try:
                ...
                for img_index, img_info in enumerate(image_list):
                    try:
                        ...
                        # Apply OCR
                        text = pytesseract.image_to_string(imgage, lang=self.ocr_lang).strip()
                        ...
                        print(f"  Warning: Could not OCR image {img_index} on page {page_num}: {e}")
                        continue
