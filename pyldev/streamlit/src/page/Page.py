...
class Page(ABC):
    ...
    def update_progress_bar(self, path: str, progress_bar: delta_generator, watched_thread: Thread, progress_bar_name: str = None):
        """
        Reads a txt file and updates a progress bar. The file must be of format 'action_name, progress_float'.
        """

        if progress_bar_name is None: progress_bar_name = watched_thread.name
        ...
        while (progress_percentage < 1):# and watched_thread.is_alive():
            ...
            try:
                ...
                if (progress_percentage < 1) and not watched_thread.is_alive():
                    print(f"Thread {watched_thread.getName()} died while progess percentage was at {round(progress_percentage, 3)}")
                    break
            except:
                None
