...
class PageLogin(Page):
    ...
    def _load_page(self):
        ...
        with st.form(key="login", border=False):
            ...
            submit_button = st.form_submit_button(label='Login', type="primary", use_container_width=True)
            if submit_button:
        ...
        with st._bottom:
            ...
            with col1:
                if st.button(label=":grey_question:", help="About my data"):
                    ...
                    st.rerun()
            with col3:
                st.markdown(f"[Check out PyYel!](https://github.com/PyYel/PyYel-DevOps/)", unsafe_allow_html=True)
    ...
    def _create_user_session(self):
        ...
        os.makedirs(os.path.join(MAIN_DIR, "data", "user", st.session_state["user_hash"]), exist_ok=True)
